...
function envPrint($id,$matrix,$times,$lossMit){
...
function buildFromPacket($packet,$times,$mail){
	...
	while ($i < 6){$i++;
		if ($d["name$i"]){
			if ($d[lossMit] == 'FINAL'){
				envPrint($toCounty,"$packet-$i",$times,$d[lossMit]);
			}
			envPrint($toAttorney,"$packet-$i",$times,$d[lossMit]);
		}
		if ($mail != ''){
			foreach(range('a','e') as $letter){
				...
				if ($d["address$var"]){
					$var=strtoupper($var);
					if ($d[lossMit] == 'FINAL'){
						envPrint($toCounty,"$packet-$var",$times,$d[lossMit]);
					}
					envPrint($toAttorney,"$packet-$var",$times,$d[lossMit]);
				}
			}
			if ($d[pobox]){
				$var=$i."PO";
	...
	while ($i < 6){$i++;
		...
		if (trim($d["add$i"]) != ''){
			if ($lossMit != 'PRELIMINARY'){
				envPrint($toCounty,"$packet-$i",$times,$d1[lossMit]);
			}
			envPrint($toAttorney,"$packet-$i",$times,$d1[lossMit]);
		}
		if ($mail != ''){
			foreach(range('a','e') as $letter){
				$var=$i.$letter;
				if (trim($d["add$var"]) != ''){
					if ($lossMit != 'PRELIMINARY'){
						envPrint($toCounty,"$packet-$var",$times,$d1[lossMit]);
					}
					envPrint($toAttorney,"$packet-$var",$times,$d1[lossMit]);
				}
			}
			$var=$i."PO";
			if (trim($d["add$var"]) != ''){
...
?>
<script>document.title='HB472 White Envelopes';</script>
