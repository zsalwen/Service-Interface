...
if ($_POST[served] == "INVALID"){ 
	timeline($packet,$_COOKIE[psdata][name]." Entered Invalid Address for ".$ddr["name$defendant"]);
	$type='Invalid Address';
	$sort=1;
	mail($_POST[to], $_POST[subject], $_POST[msg]);
} 
if ($_POST[served] == "FIRST EFFORT"){ 
	timeline($packet,$_COOKIE[psdata][name]." Entered First Effort for ".$ddr["name$defendant"]);
	$type='Attempted Service';
	$sort=0;
} 
if ($_POST[served] == "SECOND EFFORT"){ 
	timeline($packet,$_COOKIE[psdata][name]." Entered Second Effort for ".$ddr["name$defendant"]);
	$type='Attempted Service';
	$sort=0;
} 
if ($_POST[served] == "POSTING DETAILS"){ 
	timeline($packet,$_COOKIE[psdata][name]." Entered Posting Details for ".$ddr["name$defendant"]);
	$type='Posted Papers';
	$sort=0;
...
if ($_POST[served] == "BORROWER"){ 
	timeline($packet,$_COOKIE[psdata][name]." Entered Delivery Details - Served Defendant for ".$ddr["name$defendant"]);
...
if ($_POST[served] == "NOT BORROWER"){ 
	timeline($packet,$_COOKIE[psdata][name]." Entered Delivery Details - Substitute Service for ".$ddr["name$defendant"]);
...
if ($_POST[served] == "CHANGE SIGNATORY"){
	...
	if($_COOKIE[psdata][level]=="Operations"){
		if ($defendant == 'ALL'){
			...
			while ($i < 6){$i++;
				if ($ddr["name$i"]){
					$qd=@mysql_query("INSERT into ps_history (packet_id, defendant_id, action_type, action_str, serverID, recordDate, wizard, resident, residentDesc, address, sort_value, actionDate ) VALUES ('$packet', '$i', '$type', '$history', '$_POST[opServer]', NOW(), '$_POST[served]', '$_POST[name]', '$defendant_detail', '$_POST[serve_address]', '$sort', '$_POST[dt]')") or die("Query: $qd<br>".mysql_error());
				}
			}
		}else{
		...
		if ($defendant == 'ALL'){
			...
			while ($i < 6){$i++;
				if ($ddr["name$i"]){
					$qd=@mysql_query("INSERT into ps_history (packet_id, defendant_id, action_type, action_str, serverID, recordDate, wizard, resident, residentDesc, address, actionDate ) VALUES ('$packet', '$i', '$type', '$history', '$server', NOW(), '$_POST[served]', '$_POST[name]', '$defendant_detail', '$_POST[serve_address]', '$_POST[dt]')") or die("Query: $qd<br>".mysql_error());
				}
			}
		}else{
			$qd=@mysql_query("INSERT into ps_history (packet_id, defendant_id, action_type, action_str, serverID, recordDate, wizard, resident, residentDesc, address, actionDate ) VALUES ('$packet', '$defendant', '$type', '$history', '$server', NOW(), '$_POST[served]', '$_POST[name]', '$defendant_detail', '$_POST[serve_address]', '$_POST[dt]')") or die("Query: $qd<br>".mysql_error());
		}
	}
}
