...
<? }elseif($_POST[service_type] == "CHANGE SIGNATORY"){
	...
	echo "<br>".$instructions;
	$qs="SELECT * FROM ps_users WHERE id='".$_COOKIE[psdata][user_id]."'";
	$rs=@mysql_query($qs) or die(mysql_error());
	$ds=mysql_fetch_array($rs,MYSQL_ASSOC);
	$useStored="inputUpdate('$ds[phone]','server_phone');inputUpdate('$ds[address]','server_add');inputUpdate('$ds[city]','server_city');inputUpdate('$ds[state]','server_state');inputUpdate('$ds[zip]','server_zip');"
?>
<script>
function inputUpdate(text,field){
	document.getElementById(field).value = text;
}
</script>
<table><tr><td>1) Enter Server's Full Name</td><td><input name="server_name" /></td></tr>
<tr><td align="center" colspan="2"><input type="button" name="stored" id="stored" onclick="<?=$useStored?>" style="background-color:#CCFFFF; font-variant:small-caps; font-size:12px;" value="Use Stored Residence Information"></td></tr>
<tr><td>2) Server's Phone #</td><td><input id="server_phone" name="server_phone" /></td></tr>
<tr><td>3) Enter Address</td><td><input id="server_add" name="server_add" /></td></tr>
<tr><td>4) Enter City</td><td><input id="server_city" name="server_city" /></td></tr>
<tr><td>5) Enter State</td><td><input id="server_state" name="server_state"  size="2"/></td></tr>
<tr><td>6) Enter Zip</td><td><input id="server_zip" name="server_zip" size="5" /></td></tr></table>
...
<?=$item[history_id]?>)  <?=id2name($item[serverID])?> <?=$item[action_type]?> (<?=$item[wizard]?>)<hr /><?=$item[action_str]?><br /><? if ($item[wizard] === 'BORROWER' || $item[wizard] === 'NOT BORROWER'){ echo "RESIDENT DESCRIPTION: ".$item[residentDesc];}?><hr /><?=$item[recordDate]?>
</div>
<? } ?>
