...
if ($data[lossMit] != '' && $data[lossMit] != 'N/A - OLD L' && $data[packet_id] >= 12435){ 
	...
	$toAttorney=id2attorneyName($data[attorneys_id]);
		...
		if ($data[lossMit] == 'FINAL'){
			...
			$lossMit .= ", and a GREEN, preprinted #10 envelope addressed to '".$toCounty."'";
		}
...
<table style="border:solid 1px; font-size: 12px;" border=1 width="100%" align="center">
    ...
    <tr>
    	<td colspan="6" align="center"><div style="width:650px; text-align:left;"><div style="border:ridge 3px; font-size: 16px;"><strong style="text-decoration:underline;">ALL SERVICE MUST TAKE INTO ACCOUNT:</strong><br>'Two good faith attempts on separate days' - MD Rule 14-209(b)</div><ul><li>You cannot make two attempts at the same property within the same 24 hour period.</li>
