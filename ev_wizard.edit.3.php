...
foreach ($_POST[remove] as $key => $value){
	...
	if($_COOKIE[psdata][level]=="Operations"){
		$qd=@mysql_query("UPDATE evictionHistory SET defendant_id='$defendant', action_type='$type', action_str='$history', serverID='$_POST[opServer]', recordDate=NOW(), wizard='".$_POST["served-$key"]."', resident='".$_POST["name-$key"]."', residentDesc='".strtoupper($_POST["defendant_detail-$key"])."', address='".$_POST["serve_address-$key"]."', actionDate='".$_POST["dt-$key"]."' WHERE history_id='$key'") or die("Query: $qd<br>".mysql_error());
	}else{
