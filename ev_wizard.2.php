...
?>
<input name="service_type" value="<?=$_POST[service_type]?>" type="hidden" />
<div class="nav"><input onClick="submitLoader()" type="radio" name="served" value="NOT BORROWER" /> SERVED RESIDENT</div>
<input type="hidden" name="i" value="3">
...
<? }else{
	...
	echo "<br>".$instructions;
if ($_COOKIE[psdata][level] == "Operations"){
$history_items = @mysql_query("select * from evictionHistory where eviction_id='$packet' and defendant_id='$defendant'") or die(mysql_error());
}else{
$history_items = @mysql_query("select * from evictionHistory where eviction_id='$packet' and defendant_id='$defendant' and serverID='".$_COOKIE[psdata][user_id]."'") or die(mysql_error());
}
