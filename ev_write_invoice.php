...
function defCount($eviction_id){
	$c=0;
	$r=@mysql_query("SELECT name1, name2, name3, name4, name5, name6 from evictionPackets WHERE eviciton_id='$eviction_id'");
...
$id=$_GET[id];
$q = "SELECT * FROM evictionPackets, attorneys WHERE evictionPackets.attorneys_id = attorneys.attorneys_id AND eviction_id = '$id'";
...
$pdf->addTextWrap(305,618,300,14,'Eviction Packet: #'.$data['eviction_id']);
...
$pdf->addTextWrap(40,400,300,25,'MAIL:');
		$pdf->addTextWrap(500,400,300,25,'$'.number_format($data['bill420'],2));	
$pdf->addTextWrap(40,300,300,25,'FILE:');
		$pdf->addTextWrap(500,300,300,25,'$'.number_format($data['bill430'],2));	
...
$pdf->addTextWrap(500,162,300,10,'$'.number_format($data['bill410']+$data['bill420']+$data['bill430'],2));			
...
$q="insert into EIVC (attorneys_id,evictionID,dataFile,url,savedby,stored) values ('$attid','$id','$fname','$invoiceName','$cookieName',NOW())";
