...
$r1=@mysql_query("select address1, packet_id, name1, name2, name3, name4, name5, name6 from ps_packets ORDER BY packet_id DESC");
}else{
$r1=@mysql_query("select address1, packet_id, name1, name2, name3, name4, name5, name6 from ps_packets where ((server_id = '$server') or (server_ida = '$server') or (server_idb = '$server') or (server_idc = '$server') or (server_idd = '$server') or (server_ide = '$server')) ORDER BY packet_id DESC");
}
...
if ($r1){
	...
	if ($d1[name5]){	$option .= "<option value='$d1[packet_id]-4'>$d1[name5]</option>"; }
	if ($d1[name6]){	$option .= "<option value='$d1[packet_id]-4'>$d1[name6]</option>"; }
						$option .= "</OPTGROUP>";
	}
}
?>
