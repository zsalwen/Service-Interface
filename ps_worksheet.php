...
function rangeLinks($exStart,$exStop,$server,$idType,$table,$linkAppend){
	...
	while ($i < ($end-1)){$i++;
		...
		if ($start != $exStart && $stop != $exStop){
			$newList = "<td><div style='border: 1px solid black; width: 110px; font-size:11px;'><center><a href='http://service.mdwestserve.com/ps_worksheet.php?start=$start".$linkAppend."'>";
			$start=$start*1000;
			$stop=$stop*1000;
