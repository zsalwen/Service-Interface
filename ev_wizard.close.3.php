...
$q10="SELECT DISTINCT defendant_id FROM evictionHistory WHERE eviction_id='$packet'";
...
if (($defs >= $defCount) || $_POST[overRide] > 0){
	if($_POST[overRide] == 2){//if overRide is set to 2, then run update query, send email, etc.
		include 'ev_wizard.close.3a.php';
		...
		if ($ddr[service_status] == 'PERSONAL DELIVERY'){
			$q10="SELECT DISTINCT history_id FROM evictionHistory WHERE eviction_id='$packet' AND (action_type='Served Resident' OR action_type='Served Defendant'";
			$r10=mysql_query($q10) or die("Query: $q10<br>".mysql_error());
			$pd=mysql_num_rows($r10);
			include 'ev_wizard.close.3a.php';//run the update, send the email, etc.
		}elseif ($ddr[service_status] == 'MAILING AND POSTING'){
			$postingServer=$ddr[server_id];
			$q10="SELECT DISTINCT history_id FROM evictionHistory WHERE eviction_id='$packet' AND action_type='Attempted Service'";
			$r10=mysql_query($q10) or die("Query: $q10<br>".mysql_error());
			$attempts=mysql_num_rows($r10);
			$q11="SELECT DISTINCT history_id FROM evictionHistory WHERE eviction_id='$packet' AND action_type='Posted Papers' AND serverID='$postingServer'";
