...
if ($d11[needSignatory] == "checked"){
	...
	if ($d10=mysql_fetch_array($r10, MYSQL_ASSOC)){
		...
		//talk('insidenothing@gmail.com',$_COOKIE[psdata][name]." Completing Service");
			//update status
...
<? } 
if ($email > 0){
	$history=historyList($ddr[packet_id],$ddr[attorneys_id]);
	if (strpos($history,'"')){
		$history=str_replace('"','\"',$history);
	}
	$to = "Service <service@mdwestserve.com>";
	$subject = "OTD$packet, $name - Close Requested";
	$headers  = "MIME-Version: 1.0 \n";
	$headers .= "Content-type: text/html; charset=iso-8859-1 \n";
	$headers .= "From: ".$_COOKIE[psdata][name]." <".$_COOKIE[psdata][email]."> \n";
	$body ="Server $name has requested close for packet $packet.<br>
